...
public class Crad_IE3 extends BaseSpecialItemPlugin{
    ...
    public void performRightClickAction() {
        ...
        for(OfficerDataAPI member : this.player_fleet.getFleetData().getOfficersCopy()) {
            ...
            if(Objects.equals(officers.getId(), "WitchIE")){
                ...
                for (FleetMemberAPI member2 : this.player_fleet.getFleetData().getMembersListCopy()) {
                    ...
                    if(Objects.equals(officer2.getId(), "WitchIE")){
        ...
        if(!Global.getSector().getMemoryWithoutUpdate().getBoolean("$WthC_CAP")){
            Global.getSector().getPlayerFleet().getCargo().addSpecial(new SpecialItemData("Crad_IE4", "special_items"), 1.0F);
            Global.getSoundPlayer().playUISound(getSpec().getSoundId(), 1f, 1f);
            Global.getSector().getCampaignUI().getMessageDisplay().addMessage(
                    "You have temporarily put this Wraith Key into hibernation... Use it again to reawaken the link.");
        } else {
            Global.getSector().getPlayerFleet().getCargo().addSpecial(new SpecialItemData("Crad_IE3EX", "special_items"), 1.0F);
            Global.getSoundPlayer().playUISound(getSpec().getSoundId(), 1f, 1f);
            Global.getSector().getMemoryWithoutUpdate().set("$IEEX-ON", true);
            Global.getSector().getCampaignUI().getMessageDisplay().addMessage(
                    "The Wraith Key refused to hibernate... and shifted into another form...");

            List<String> Witch = new ArrayList();
            Witch.add("Pale_SENetLink");
            Witch.add("Pale_WitchIE");
            ...
            this.officer.setId("WitchIEEX");
            this.officer.setPortraitSprite(Global.getSettings().getSpriteName("intel", "WPR1"));

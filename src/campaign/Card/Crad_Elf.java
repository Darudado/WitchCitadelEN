...
public class Crad_Elf extends BaseSpecialItemPlugin{
    ...
    public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, CargoTransferHandlerAPI transferHandler, Object stackSource) {
        ...
        tooltip.addPara("Those who are reborn will still fade away once more...", WthC_ColorData.EIF_ORANGE, padS);
    }
    ...
    public void performRightClickAction() {
        ...
        FleetMemberAPI member = Global.getFactory().createFleetMember(FleetMemberType.SHIP, "WthC_hyperion_SS_S_N_New");
        Global.getSector().getPlayerFleet().getFleetData().addFleetMember(member);

        CoreReputationPlugin.CustomRepImpact impact = new CoreReputationPlugin.CustomRepImpact();
        impact.delta = 0.5f;
        ReputationActionResponsePlugin.ReputationAdjustmentResult result = Global.getSector().adjustPlayerReputation(new CoreReputationPlugin.RepActionEnvelope(CoreReputationPlugin.RepActions.CUSTOM, impact, null, null, true), WthC_Citadel.getEDEN());
        Global.getSector().getPlayerFleet().getCargo().addSpecial(new SpecialItemData("Crad_Elf2", "special_items"), 1.0F);
        Global.getSoundPlayer().playUISound(getSpec().getSoundId(), 1f, 1f);
        Global.getSector().getCampaignUI().getMessageDisplay().addMessage(
                "You have gained the approval of a witch. She has fully restored your flagship and will assist you remotely as a special officer throughout your journey.");
    }
}

...
public class Crad_RRA3 extends BaseSpecialItemPlugin{
    ...
    public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, CargoTransferHandlerAPI transferHandler, Object stackSource) {
        ...
        tooltip.addPara(strings.get("CardRe2"), pad, WthC_ColorData.HIGH_BLUE, "Wuxin You Yue");
        tooltip.addPara(strings.get("CardRe3"), padS, WthC_ColorData.HIGH_BLUE, "Enchanting Witch");
        tooltip.addPara(strings.get("CardRe4"), padS, WthC_ColorData.HIGH_BLUE, "This witch knows well how to protect herself while providing support from afar...");
        tooltip.addPara("She excels at delivering precise fire support from extreme distances and can quickly disengage from the battlefield when her ship is under significant threat.", WthC_ColorData.HIGH_BLUE, padS);
    ...
    public void performRightClickAction() {
        ...
        for(OfficerDataAPI member : this.player_fleet.getFleetData().getOfficersCopy()) {
            ...
            if(Objects.equals(officers.getId(), "WitchRRA")){
                ...
                for (FleetMemberAPI member2 : this.player_fleet.getFleetData().getMembersListCopy()) {
                    ...
                    if(Objects.equals(officer2.getId(), "WitchRRA")){
        ...
        if(!Global.getSector().getMemoryWithoutUpdate().getBoolean("$WthC_CAP")){
            Global.getSector().getPlayerFleet().getCargo().addSpecial(new SpecialItemData("Crad_RRA4", "special_items"), 1.0F);
            Global.getSoundPlayer().playUISound(getSpec().getSoundId(), 1f, 1f);
            Global.getSector().getCampaignUI().getMessageDisplay().addMessage(
                    "You have temporarily put this Wraith Key into hibernation... Use it again to reawaken the link.");
        } else {
            Global.getSector().getPlayerFleet().getCargo().addSpecial(new SpecialItemData("Crad_RRA3EX", "special_items"), 1.0F);
            Global.getSoundPlayer().playUISound(getSpec().getSoundId(), 1f, 1f);
            Global.getSector().getMemoryWithoutUpdate().set("$RRAEX-ON", true);
            Global.getSector().getCampaignUI().getMessageDisplay().addMessage(
            ...
            Witch.add("Pale_WitchRRA");
            Witch.add("Pale_combat_endurance");
            Witch.add("Pale_damage_control");
            ...
            Witch2.add("Pale_missile_specialization");
            ...
            this.officer.setId("WitchRRAEX");
            this.officer.setPortraitSprite(Global.getSettings().getSpriteName("intel", "WPY1"));
            this.officer.setName(new FullName("YouYu", "Wuxin", FullName.Gender.FEMALE));

...
public class Crad_IE extends BaseSpecialItemPlugin{
    ...
    public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, CargoTransferHandlerAPI transferHandler, Object stackSource) {
        ...
        tooltip.addPara("May you be the last mortal chosen...", WthC_ColorData.IE_WHITE, padS);
    }
    ...
    public void performRightClickAction() {
        ...
        FleetMemberAPI member = Global.getFactory().createFleetMember(FleetMemberType.SHIP, "WthC_paragon_IE_N_New");
        Global.getSector().getPlayerFleet().getFleetData().addFleetMember(member);

        CoreReputationPlugin.CustomRepImpact impact = new CoreReputationPlugin.CustomRepImpact();
        impact.delta = 0.5f;
        ReputationActionResponsePlugin.ReputationAdjustmentResult result = Global.getSector().adjustPlayerReputation(new CoreReputationPlugin.RepActionEnvelope(CoreReputationPlugin.RepActions.CUSTOM, impact, null, null, true), WthC_Citadel.getRIKA());
        Global.getSector().getPlayerFleet().getCargo().addSpecial(new SpecialItemData("Crad_IE2", "special_items"), 1.0F);

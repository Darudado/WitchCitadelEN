...
public class Crad_SSoul extends BaseSpecialItemPlugin{
    ...
    public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, CargoTransferHandlerAPI transferHandler, Object stackSource) {
        ...
        tooltip.addPara("In the void, your destined death will eventually come...", WthC_ColorData.SOUL_GREEN, padS);
    }
    ...
    public void performRightClickAction() {
        ...
        this.officer.setId("WitchSSoul");
        this.officer.setPortraitSprite(Global.getSettings().getSpriteName("intel", "WS1"));
        this.officer.setName(new FullName("Su", "Soul", FullName.Gender.FEMALE));
        this.player_fleet.getFleetData().addOfficer(this.officer);

        FleetMemberAPI member = Global.getFactory().createFleetMember(FleetMemberType.SHIP, "WthC_pegasus_IE_B_N_New");
        Global.getSector().getPlayerFleet().getFleetData().addFleetMember(member);

        CoreReputationPlugin.CustomRepImpact impact = new CoreReputationPlugin.CustomRepImpact();
        impact.delta = 0.5f;
        ReputationActionResponsePlugin.ReputationAdjustmentResult result = Global.getSector().adjustPlayerReputation(new CoreReputationPlugin.RepActionEnvelope(CoreReputationPlugin.RepActions.CUSTOM, impact, null, null, true), WthC_Citadel.getSU());
        Global.getSector().getPlayerFleet().getCargo().addSpecial(new SpecialItemData("Crad_SSoul2", "special_items"), 1.0F);
        Global.getSoundPlayer().playUISound(getSpec().getSoundId(), 1f, 1f);
        Global.getSector().getCampaignUI().getMessageDisplay().addMessage(
                "You have gained the recognition of a witch. She has helped fully restore your flagship and will assist you remotely as a special officer throughout the journey ahead.");
    }
}

...
//从军用市场修改
public class Between_Visits extends BaseSubmarketPlugin {
    public static Logger log = Global.getLogger(Between_Visits.class);
    private RepLevel minStanding;

    public Between_Visits() {
    ...
    public String getName() {
        return this.submarket.getFaction().getId().equals("Pale_Phantom") ? "Audience Chamber" :  "";
    }
    ...
    public boolean isEnabled(CoreUIAPI ui) {
        if (ui.getTradeMode() == CoreUITradeMode.SNEAK) {
            ...
            RepLevel level = RepLevel.getLevelFor(WthC_Citadel.getROMENA().getRelToPlayer().getRel());
    ...
    public String getTooltipAppendix(CoreUIAPI ui) {
        if (!this.isEnabled(ui)) {
            return "Requirement: Personal relationship with the Pale King greater than 60";
        } else {

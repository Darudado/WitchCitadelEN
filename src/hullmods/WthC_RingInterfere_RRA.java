...
//部分格式来源于北极星
public class WthC_RingInterfere_RRA extends BaseHullMod {
    ...
    public static final float PEAK_BONUS_PERCENT2 = 25f;
    public static float DEGRADE_REDUCTION_PERCENT = 25f;

    public static final float DAMAGE_TAKEN_MULT = 0.95f;
    public static final float DAMAGE = 0.95f;
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        if (ship == null) {
            ...
            tooltip.addPara("This Wraith Interference Protocol contains partial stellar consciousness information derived from the Eden Witch", WthC_ColorData.IE_WHITE, pad);
            tooltip.addPara("Installing this protocol will make the ship's AI system choose more conservative combat methods...", WthC_ColorData.IE_WHITE, padS);
        ...
        if (!ship.getVariant().hasHullMod("crimsonmodificationssoul")) {
            ...
            tooltip.addPara("This Wraith Interference Protocol contains partial stellar consciousness information derived from the Eden Witch", WthC_ColorData.IE_WHITE, pad);
            tooltip.addPara("Installing this protocol will make the ship's AI system choose more conservative combat methods...", WthC_ColorData.IE_WHITE, padS);
            tooltip.addPara("She cannot protect a defective product forever...", WthC_ColorData.DEEP_BLUE, pad);
            if(count >= 1) {
                tooltip.addPara("                                        "+"Even so... lasting for countless millennia is no difficult feat...", WthC_ColorData.IE_WHITE, padS);
            }
        ...
        tooltip.addPara("     %s " + strings.get("RingInterfere_RRA_TEXT1"), padS, Misc.getPositiveHighlightColor(), "#", "25%");
        tooltip.addPara("     %s " + strings.get("RingInterfere_RRA_TEXT2"), padS, Misc.getNegativeHighlightColor(), "#", "5%");
        if(count >= 1) {
            tooltip.addPara("     %s " + strings.get("RingInterfere_RRA_TEXTEX2"), padS, WthC_ColorData.HIGH_BLUE, "#", "5%");
        }
        if(count >= 1) {
        ...
        tooltip.addPara(" %s " + strings.get("CrimsonModification_TEXT000"), padS, Misc.getNegativeHighlightColor(), "#");
        }
    }
    ...
    public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
        ...
        stats.getPeakCRDuration().modifyPercent(id, PEAK_BONUS_PERCENT2);
    ...
    public void applyEffectsAfterShipCreation(ShipAPI ship, String id) {
        ...
        while (i$.hasNext()) {
            ...
            if (ship.getVariant().getHullMods().contains(tmp)) {
                MagicIncompatibleHullmods.removeHullmodWithWarning(ship.getVariant(), tmp, "ringinterfererra");
            }
        }
            while (i$2.hasNext()) {
                ...
                if(count2 >= 2){
                    if (ship.getVariant().getHullMods().contains(tmp2)) {
                        MagicIncompatibleHullmods.removeHullmodWithWarning(ship.getVariant(), tmp2, "ringinterfererra");
                    }
                }
            }

...
//部分格式来源于北极星
public class WthC_RingInterfere_Dust extends BaseHullMod {
    ...
    public static final float DAMAGE_TAKEN_MULT = 1.15f;
    public static final float DAMAGE = 1.10f;
    public static final float DAMAGE2 = 1.15f;
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        if (ship == null) {
            ...
            tooltip.addPara("This Wraith Interference Protocol contains partial stellar consciousness information derived from the Dust Witch", WthC_ColorData.IE_WHITE, pad);
            tooltip.addPara("Installing this protocol will make the ship's AI system choose more risky offensive methods... even if it puts the ship in danger", WthC_ColorData.IE_WHITE, padS);
        ...
        if (!ship.getVariant().hasHullMod("crimsonmodificationssoul")) {
            ...
            tooltip.addPara("This Wraith Interference Protocol contains partial stellar consciousness information derived from the Dust Witch", WthC_ColorData.IE_WHITE, pad);
            tooltip.addPara("Installing this protocol will make the ship's AI system choose more risky offensive methods... even if it puts the ship in danger", WthC_ColorData.IE_WHITE, padS);
            tooltip.addPara("Even if everything burns to ashes, what's lost will never return...", WthC_ColorData.DUST_RED, pad);
            if(count >= 1) {
                tooltip.addPara("                                                "+"But you never wanted them back, did you?", WthC_ColorData.EIF_ORANGE, padS);
            }
        ...
        if(count == 0) {
            tooltip.addPara("     %s " + strings.get("RingInterfere_Dust_TEXT1"), padS, Misc.getPositiveHighlightColor(), "#", "10%");
        }
        if(count >= 1) {
            tooltip.addPara("     %s " + strings.get("RingInterfere_Dust_TEXT1"), padS, WthC_ColorData.HIGH_BLUE, "#", "15%");
        }
        tooltip.addPara("     %s " + strings.get("RingInterfere_Dust_TEXT2"), padS, Misc.getNegativeHighlightColor(), "#", "15%");
    ...
    public void applyEffectsAfterShipCreation(ShipAPI ship, String id) {
        ...
        while (i$.hasNext()) {
            ...
            if (ship.getVariant().getHullMods().contains(tmp)) {
                MagicIncompatibleHullmods.removeHullmodWithWarning(ship.getVariant(), tmp, "ringinterferedust");
            }
        }
            while (i$2.hasNext()) {
                ...
                if(count2 >= 2){
                    if (ship.getVariant().getHullMods().contains(tmp2)) {
                        MagicIncompatibleHullmods.removeHullmodWithWarning(ship.getVariant(), tmp2, "ringinterferedust");
                    }
                }
            }

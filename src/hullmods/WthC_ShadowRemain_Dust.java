...
//部分格式来源于北极星
public class WthC_ShadowRemain_Dust extends BaseHullMod {
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        if (ship != null) {
            ...
            tooltip.addPara("     %s " + strings.get("ShadowRemain_Dust_TEXT0"), pad, Misc.getHighlightColor(), "#","Hull integrity reduction","Damage dealt");
            tooltip.addPara("         %s " + strings.get("ShadowRemain_Dust_TEXT1"), padS, WthC_ColorData.HIGH_BLUE,"#","0%","25%");
            tooltip.addPara("         %s " + strings.get("ShadowRemain_Dust_TEXT2"), padS, WthC_ColorData.HIGH_BLUE,"#","10%");
            ...
            tooltip.addSectionHeading("Additional Notes", Alignment.TMID, 10.0F);
    ...
    public void advanceInCombat(ShipAPI ship, float amount) {
        ...
        float points = ((ship.getMaxHitpoints() - ship.getHitpoints()) / ship.getMaxHitpoints());
        float DamageUp = Math.min(25f, points * 28F);
        int DamageUpInt = Math.round(DamageUp);

        if (ship.isAlive()){
            ship.getMutableStats().getDamageToMissiles().modifyPercent(id123, DamageUp);
            ship.getMutableStats().getDamageToFighters().modifyPercent(id123, DamageUp);
            ship.getMutableStats().getDamageToFrigates().modifyPercent(id123, DamageUp);
            ship.getMutableStats().getDamageToDestroyers().modifyPercent(id123, DamageUp);
            ship.getMutableStats().getDamageToCruisers().modifyPercent(id123, DamageUp);
            ship.getMutableStats().getDamageToCapital().modifyPercent(id123, DamageUp);
        }
        if (ship == Global.getCombatEngine().getPlayerShip()) {
            Global.getCombatEngine().maintainStatusForPlayerShip("WthC_ShadowRemain_Dust", "graphics/icons/hullsys/GatherStarCore.png", "Shadow Remnant - Dust", "Damage dealt increased by " + DamageUpInt + " " + "%", false);
        }
    ...
    public void applyEffectsAfterShipCreation(ShipAPI ship, String id) {
        ...
        for (String tmp : BLOCKED_HULLMODS) {
            if (ship.getVariant().getHullMods().contains(tmp)) {
                MagicIncompatibleHullmods.removeHullmodWithWarning(ship.getVariant(), tmp, "shadowremaindust");
            }
        }
    ...
    public String getUnapplicableReason(ShipAPI ship) {
        ...
        static {
        BLOCKED_HULLMODS.add("ringinterferedust");
        BLOCKED_HULLMODS.add("crimsonmodificationdust");
        BLOCKED_HULLMODS.add("shadowremaincross");

...
//部分格式来源于北极星
public class WthC_RingInterfere_SSoul extends BaseHullMod {
    ...
    public static float TIME2 = 10f;
    public static float DAMAGE1 = 1.05f;
    public static float DAMAGE2 = -25f;
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        if (ship == null) {
            ...
            tooltip.addPara("This Ring Spirit interference protocol contains partial stellar consciousness information from the Soul Witch", WthC_ColorData.IE_WHITE, pad);
            tooltip.addPara("Installing this protocol will cause the ship's AI system to apply part of the ship's equipment to subtle spacetime interference.", WthC_ColorData.IE_WHITE, padS);
            tooltip.addPara("For certain reasons, please move to the loadout interface to view detailed effects.", WthC_ColorData.HIGH_BLUE, padL);
        }
        if (ship != null) {
        ...
        if (!ship.getVariant().hasHullMod("crimsonmodificationssoul")) {
            ...
            tooltip.addSectionHeading("Introduction", Alignment.TMID, 5.0F);
            tooltip.addPara("This Wraith Interference Protocol contains partial stellar consciousness information derived from the Spirit Witch", WthC_ColorData.IE_WHITE, pad);
            tooltip.addPara("Installing this protocol will make the ship's AI system apply some ship equipment to minor spacetime interference.", WthC_ColorData.IE_WHITE, padS);
            tooltip.addPara("After all, I never had to care whether you lived or died, did I?", WthC_ColorData.SOUL_GREEN, pad);
            if(count >= 1) {
                tooltip.addPara("                                                "+"Since after death, what you face is still yourself...", WthC_ColorData.CROSS_GARY, padS);
            }
        ...
        if(count == 0) {
            tooltip.addPara("     %s " + strings.get("RingInterfere_SSoul_TEXT1"), padS, Misc.getPositiveHighlightColor(), "#", "5%");
            tooltip.addPara("     %s " + strings.get("RingInterfere_SSoul_TEXT2"), padS, Misc.getNegativeHighlightColor(), "#", "25%");
        }
        if(count >= 1) {
            tooltip.addPara("     %s " + strings.get("RingInterfere_SSoul_TEXT1"), padS, WthC_ColorData.HIGH_BLUE, "#", "10%");
            tooltip.addPara("     %s " + strings.get("RingInterfere_SSoul_TEXT2"), padS, Misc.getNegativeHighlightColor(), "#", "25%");
            tooltip.addPara("     %s " + strings.get("RingInterfere_SSoul_TEXT3"), padS, WthC_ColorData.HIGH_BLUE, "#", "5%");
        }
    ...
    public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
        ...
        if(count == 0) {
            ...
            stats.getHullBonus().modifyPercent(id, DAMAGE2);
        }

        if(count >= 1) {
            stats.getTimeMult().modifyPercent(id, TIME2);
            stats.getHullBonus().modifyPercent(id, DAMAGE2);
            stats.getShieldDamageTakenMult().modifyMult(id, DAMAGE1);
        }
    ...
    public void advanceInCombat(ShipAPI ship, float amount) {
        ...
        ship = (ShipAPI)ship.getMutableStats().getEntity();
        player = ship == Global.getCombatEngine().getPlayerShip();
        if (sMod) {
            if (player) {
                Global.getCombatEngine().getTimeMult().modifyMult(id, 0.66F);
            } else {
    ...
    public void applyEffectsAfterShipCreation(ShipAPI ship, String id) {
        ...
        while (i$.hasNext()) {
            ...
            if (ship.getVariant().getHullMods().contains(tmp)) {
                MagicIncompatibleHullmods.removeHullmodWithWarning(ship.getVariant(), tmp, "ringinterferessoul");
            }
        }
            while (i$2.hasNext()) {
                ...
                if(count2 >= 2){
                    if (ship.getVariant().getHullMods().contains(tmp2)) {
                        MagicIncompatibleHullmods.removeHullmodWithWarning(ship.getVariant(), tmp2, "ringinterferessoul");
                    }
                }
            }

...
//部分格式来源于北极星
public class WthC_ShadowRemain_SSoul extends BaseHullMod {
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        if (ship != null) {
            ...
            tooltip.addPara("     %s " + strings.get("ShadowRemain_SSoul_TEXT0"), pad, Misc.getHighlightColor(), "#","Time flow rate","Flux level");
            tooltip.addPara("         %s " + strings.get("ShadowRemain_SSoul_TEXT1"), padS, WthC_ColorData.HIGH_BLUE,"#","0%","-25%");
            tooltip.addPara("         %s " + strings.get("ShadowRemain_SSoul_TEXT2"), padS, WthC_ColorData.HIGH_BLUE,"#","100%","+33%");
    ...
    public void advanceInCombat(ShipAPI ship, float amount) {
        ...
        float points = (-25 + (0.58F * (ship.getFluxLevel() * 100)));
        int TimeUpInt = Math.round(points);

        if (ship.isAlive()){
            ship.getMutableStats().getTimeMult().modifyPercent(id123, TimeUpInt);
        }
        if (ship == Global.getCombatEngine().getPlayerShip()) {
            Global.getCombatEngine().maintainStatusForPlayerShip("WthC_ShadowRemain_SSoul", "graphics/icons/hullsys/GatherStarCore.png", "Shadow Remnant - Soul", "Time flow rate changed by " + TimeUpInt + " " + "%", TimeUpInt < 0);
    ...
    public void applyEffectsAfterShipCreation(ShipAPI ship, String id) {
        ...
        for (String tmp : BLOCKED_HULLMODS) {
            if (ship.getVariant().getHullMods().contains(tmp)) {
                MagicIncompatibleHullmods.removeHullmodWithWarning(ship.getVariant(), tmp, "shadowremainssoul");
            }
        }
    ...
    public String getUnapplicableReason(ShipAPI ship) {
        ...
        static {
        BLOCKED_HULLMODS.add("ringinterferessoul");

...
import java.util.*;
//主要来自于北极星WeaponPlugin，你懂的...缝合小子。
public class WthC_WeaponPlugin extends BaseEveryFrameCombatPlugin {
	private static final String DATA_KEY = "WthC_WeaponPlugin";
	...
	public void advance(float amount, List<InputEventAPI> events) {
		...
		for (DamagingProjectileAPI projectile : engine.getProjectiles()) {
			...
			switch (projectileID) {
				case "WthC_interferometerhyperbolic_shot":
				case "WthC_interferometerresonant_shot":
				case "WthC_interferometerdegeneracy_shot":
			...
			while (iter.hasNext()) {
				...
				if (tick <= 0f) {
					...
					for (ShipAPI target : CombatUtils.getShipsWithinRange(location, data.radius)) {
						if (target.getOwner() != data.source.getOwner()) {
							float damage = target.getVelocity().length() * 0.15f + Math.abs(target.getAngularVelocity()) * 0.1f + 3f;
							Vector2f damageLocation = WthC_Util.getShipCollisionPoint(location, target.getLocation(), target);

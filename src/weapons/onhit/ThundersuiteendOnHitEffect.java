...
//从离子脉冲特效修改
public class ThundersuiteendOnHitEffect implements OnHitEffectPlugin {
    public ThundersuiteendOnHitEffect() {
    }
    ...
    public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target, Vector2f point, boolean shieldHit, ApplyDamageResultAPI damageResult, CombatEngineAPI engine) {
        if (target instanceof ShipAPI) {
            if (shieldHit) {
                ...
                if (Math.random() <= (double)(empChanceBase * 0.66F)) {
                    float emp = projectile.getEmpAmount()* 0.00F;
                    float dam = projectile.getDamageAmount()* 0.66F;
